...
describe('Home', () => {
  it('should be able to render new added tasks', () => {
    ...
    const inputElement = getByPlaceholderText('Thêm công việc mới...');

    expect(getByText('0 công việc'));

    fireEvent.changeText(inputElement, 'Công việc 1');
    fireEvent(inputElement, 'submitEditing');
    
    expect(getByText('Công việc 1'));
    expect(getByText('1 công việc'));

    fireEvent.changeText(inputElement, 'Công việc 2');
    fireEvent(inputElement, 'submitEditing');

    expect(getByText('Công việc 1'));
    expect(getByText('Công việc 2'));
    expect(getByText('2 công việc'));
  });
  ...
  it('should be able to render tasks as done and undone', () => {
    ...
    const taskElement = getByText('Công việc 1');
  ...
  it('should be able to remove tasks after the trash icon was pressed', async () => {
    ...
    expect(queryByText('Công việc 1')).toBeNull();
    expect(getByText('Công việc 2'));
    expect(getByText('1 công việc'));
  });
})